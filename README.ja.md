...
[序文を読む](content/ja/00-preface.md) · [一枚で見る全体図](content/ja/17-one-page.md) · [Windows 版](https://github.com/xialuyu5-oss/dual-quotient-manual/releases/tag/v0.4.0)
...
**[実践ワークショップを開く](https://xialuyu5-oss.github.io/dual-quotient-manual/ja/workshop/)**。上部ナビゲーションからも移動できます。既存の手冊本文はそのままに、新しい内容を独立ページへ追加していきます。9 言語すべてに対応します。

- **8 件の架空ケース**：直前の追加要望、人前での批判、遅延報告、貢献の帰属、傾聴、手助けの限界、チームの意見対立、埋没費用。対応を選ぶと利点・代償・条件を確認でき、関連シートを開けます。
- **4 種の記入式シート**：意思決定ログ、感情日記、難しい対話の準備、利害と境界線のマップ。各シートの下書きを自動保存し、記録の保存・振り返り・読み込み、Markdown 出力、JSON バックアップの入出力ができます。
- **6 つのテーマ**：立場とニーズ、インセンティブ、決定権、交渉の選択肢、自分の行動の境界線、相談先への引き上げと撤退。各テーマに考え方、三つの問い、具体例、適用上の留意点を収録します。

記入内容は現在のブラウザー内だけに保存され、送信されません。言語間で記録を共有しますが、入力した文章は翻訳されません。拡張方法は [工坊の保守ガイド](docs/workshop-content.md) を参照してください。

## Windows ポータブル版

[v0.4.0 Windows x64 版をダウンロード](https://github.com/xialuyu5-oss/dual-quotient-manual/releases/tag/v0.4.0)し、`windows-x64.zip` を**すべて展開**してから、**双商训练手册.exe** をダブルクリックします。既定のブラウザーで開きます。使用中は起動ウィンドウを残してください。閉じるとローカルサービスも終了します。
...
- 未提出の自測回答は永続保存しません。端末・ブラウザー・アドレス間の自動同期はなく、サイトデータを消すと記録も消えます。ワークショップの下書きと記録はバックアップの入出力に対応します。自測と日課の履歴は未対応です。
