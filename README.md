...
**[直接打开实践工坊](https://xialuyu5-oss.github.io/dual-quotient-manual/zh-CN/workshop/)**，也可从顶部导航进入。原有手册页面保持原有内容，新增内容在工坊持续扩展，九种语言均可使用。

- **8 个场景案例**：临时加需求、会议批评、延期预警、贡献归属、倾听、拒绝过度帮忙、团队分歧与沉没成本。选择回应后查看收益、代价和适用条件，并打开对应练习表。案例为常见情境的虚构示例。
- **4 份可填写练习表**：决策日志、情绪日记、困难对话准备表、利益与边界地图。各有自动保存的草稿，支持保存历史快照、回看与载入、Markdown 导出、JSON 备份导入导出。
- **6 个专题**：需要与立场、行为激励、决定权、协商选项、行动边界、升级与退出。每篇包含核心观点、三个问题、例子与适用边界。

填写内容仅存于当前浏览器，不上传；切换语言共用记录，用户原文保持不变。内容扩展方法见 [工坊维护说明](docs/workshop-content.md)。

## Windows 免安装版

**[下载 v0.4.0 Windows 免安装包](https://github.com/xialuyu5-oss/dual-quotient-manual/releases/tag/v0.4.0)**
...
- 启动窗口跟随 Windows 界面语言；网页顶部可随时切换九种语言。
...
| 德语 · Deutsch | [Öffnen](https://xialuyu5-oss.github.io/dual-quotient-manual/de/) | [Lesen](content/de/) |
| 俄语 · Русский | [Открыть](https://xialuyu5-oss.github.io/dual-quotient-manual/ru/) | [Читать](content/ru/) |
| 韩国语 · 한국어 | [打开](https://xialuyu5-oss.github.io/dual-quotient-manual/ko/) | [阅读](content/ko/) |
| 西班牙语 · Español | [Abrir](https://xialuyu5-oss.github.io/dual-quotient-manual/es/) | [Leer](content/es/) |
...
| `npm run test:i18n` | 检查九套文案、占位符、18 篇章节及稳定路径 |
...
- 不同设备、浏览器或站点地址之间不会自动同步；清除站点数据会丢失记录。实践工坊支持草稿与练习记录的备份导入导出，自测和日课暂不支持备份。
...
```text
content/                          简体中文原文及八个译文子目录，每种 18 篇
src/
├── app/
│   ├── [[...route]]/             旧中文路径、九语言静态路由与页面元数据
...
- 新增或修改章节时，同步更新八个语言子目录，保持 `slug`、`part`、`level`、`order` 一致。界面词典位于 `src/lib/i18n/dictionaries/`，命名占位符应保持一致。构建会先检查所有九种语言，缺失不会静默回退成中文。
