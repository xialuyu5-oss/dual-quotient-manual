...
**[直接開啟實踐工坊](https://xialuyu5-oss.github.io/dual-quotient-manual/zh-TW/workshop/)**，也可從頂部導覽進入。原有手冊內容維持不變，新內容在工坊持續擴展，九種語言皆可使用。

- **8 個情境案例**：臨時追加需求、會議批評、延期預警、貢獻歸屬、傾聽、拒絕過度幫忙、團隊分歧與沉沒成本。選擇回應後查看利弊與適用條件，並開啟對應練習表。案例為常見情境的虛構示例。
- **4 份可填寫練習表**：決策日誌、情緒日記、困難對話準備表、利益與邊界地圖。各有自動儲存的草稿，可儲存歷史紀錄、回顧與載入、匯出 Markdown、匯入或匯出 JSON 備份。
- **6 個專題**：需要與立場、行為誘因、決定權、協商選項、行動邊界、升級處理與退出。每篇包含核心觀點、三個問題、例子與適用邊界。

填寫內容僅存於目前瀏覽器，不會上傳；切換語言共用紀錄，使用者原文保持不變。擴展方式見 [工坊維護說明](docs/workshop-content.md)。

## Windows 免安裝版

**[下載 v0.4.0 Windows 免安裝包](https://github.com/xialuyu5-oss/dual-quotient-manual/releases/tag/v0.4.0)**
...
- 不同裝置、瀏覽器或網站位址之間不會自動同步；清除網站資料會遺失紀錄。實踐工坊支援草稿與練習紀錄的備份匯入匯出，自測與日課暫不支援備份。
