...
import type { ChapterData } from "@/lib/page-data";
...
export default function ChapterPage({slug, chapter, prev, next, groups}: ChapterData) {
  const { t, tx } = useI18n();
          ...
          <article className="mx-auto max-w-3xl">
            <header className="mb-10">
              <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                <span className="hidden lg:inline">{tx(PARTS[chapter.part].title)}</span>
                ...
                <span className="ml-auto inline-flex items-center gap-1">
                  ...
                  {t("约 {minutes} 分钟", {minutes: chapter.readingMinutes})}
                </span>
              </div>
            ...
            {(slug === "know-thyself" || slug === "practice-plan") && (
              <div className="mt-8 rounded-lg bg-paper-deep p-5 text-sm">
                {slug === "know-thyself" ? (
                  <p>{t("本章配套的自测在")}<Link href="/assessment" className="mx-1 font-medium text-primary underline underline-offset-4">{t("「自测」页面")}</Link>{t("，二十四题，约五分钟。")}</p>
                ) : (
                  <p>{t("本章的日课与周课可以在")}<Link href="/practice" className="mx-1 font-medium text-primary underline underline-offset-4">{t("「日课」页面")}</Link>{t("逐项打勾，记录会保存在本机浏览器里。")}</p>
                )}
              </div>
            )}
          </article>
  ...
  return (
    ...
    >
      ...
      <span className="font-serif text-sm font-medium">
        <span className="mr-1.5 text-muted-foreground">{tx(chapter.label)}</span>
        {tx(chapter.title)}
      </span>
    </Link>
  );
