    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      ...
      <section className="grid items-center gap-10 py-14 sm:py-20 lg:grid-cols-[minmax(0,1fr)_auto] lg:gap-16">
        <div className="max-w-2xl">
          <p className="mb-4 font-serif text-sm tracking-[0.3em] text-primary">
            双商训练手册
          </p>
          <h1 className="font-serif text-4xl font-semibold leading-[1.25] tracking-wide text-balance sm:text-5xl lg:text-6xl">
            从聪明到智慧，
            <br />
            从术器到道法。
          </h1>
          <p className="mt-6 text-lg leading-relaxed text-muted-foreground">
            聪明是解题的速度，智慧是选题与承受的能力。这本手册不再给你更多技巧，而是把智商与情商放进同一张
            <span className="font-serif text-foreground">「道、法、术、器」</span>
            的地图，让技巧被消化成判断力。
          </p>
        ...
        >
          {rows.map((level) => (
            <div
              key={level}
              className={cn(
                "flex w-16 flex-col items-center justify-between rounded-md bg-card py-5 ring-1 ring-border",
              )}
            >
              <span
                className={cn(
              ...
              <span className="writing-vertical mt-4 font-serif text-xs tracking-[0.35em] text-muted-foreground">
                {LEVELS[level].short}·{LEVELS[level].verb}
              </span>
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <DefinitionCard
          term="聪明"
          gloss="解题的能力"
          body="处理信息的速度、广度与技巧。题目在那里，你解得快、解得巧、解得漂亮。它是术与器的极致。"
          tone="muted"
        />
        <DefinitionCard
          term="智慧"
          gloss="选题与承受的能力"
          body="知道什么值得做、知道自己不知道什么，并且能与结果和平共处。它是道与法的开始。"
          tone="primary"
        />
      </section>
      ...
      <section className="mt-20">
        <SectionHeading
          kicker="一张地图"
          title="双商 × 四层：八个格子"
          description="器可传，术可练，法可立，道可养。学从器入，用从道出。每个格子是一章，点开即读。"
        />
        <div className="overflow-hidden rounded-xl bg-card ring-1 ring-border">
          <div className="grid grid-cols-[4.5rem_1fr_1fr] sm:grid-cols-[9rem_1fr_1fr]">
            ...
            <div className="border-b border-l border-border bg-paper-deep p-3 sm:p-4">
              <p className="font-serif text-sm font-semibold sm:text-base">智商之路</p>
              <p className="hidden text-xs text-muted-foreground sm:block">从聪明到智慧</p>
            </div>
            <div className="border-b border-l border-border bg-paper-deep p-3 sm:p-4">
              <p className="font-serif text-sm font-semibold sm:text-base">情商之路</p>
              <p className="hidden text-xs text-muted-foreground sm:block">从技巧到心性</p>
            </div>
      ...
      <section className="mt-20">
        <SectionHeading
          kicker="四个阶段"
          title="你现在站在哪一层"
          description="每个阶段的药都不同。聪明人最常犯的错，是在所有阶段都吃同一种药——再学一点新东西。"
        />
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          不确定自己在哪一阶？
          <Link href="/assessment" className="ml-1 font-medium text-primary underline underline-offset-4">
            五分钟自测
          </Link>
          会给你两条路各自的定位，以及你的「错位」形状。
        </p>
      </section>

      {/* How to use */}
      <section className="mt-20">
        <SectionHeading kicker="如何使用" title="三步开始" />
          ...
          {STEPS.map((step, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">第 {index + 1} 步</p>
      ...
      <section className="mt-20 mb-8 grid gap-8 lg:grid-cols-[1fr_2fr]">
        <div>
          <SectionHeading
            kicker="目录"
            title="十八篇"
            description="序、地图三章、两条路各四章、合一四章、附录两篇。通读约两小时，修炼以年计。"
          />
          <Button variant="outline" nativeButton={false} render={<Link href="/manual" />}>
            完整目录
            <ArrowRightIcon data-icon="inline-end" />
          </Button>
        </div>
...
function SectionHeading({
  ...
  return (
    <div className="mb-6">
      <p className="mb-1 font-serif text-xs tracking-[0.3em] text-primary">{kicker}</p>
...
function DefinitionCard({
  ...
  return (
    ...
    >
      ...
      <p className="font-serif text-xs tracking-[0.3em] opacity-80">{gloss}</p>
      <h3 className="mt-1 font-serif text-3xl font-semibold">{term}</h3>
      <p
        className={cn(
...
function MatrixCell({
  ...
  return (
    ...
    >
      <span className="font-serif text-xs text-muted-foreground">{chapter.label}</span>
