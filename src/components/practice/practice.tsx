    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-8">
        ...
        <section className="rounded-xl bg-card ring-1 ring-border">
          <header className="flex flex-wrap items-end justify-between gap-3 border-b border-border px-5 py-4 sm:px-6">
            <div>
              <p className="text-xs tracking-widest text-muted-foreground">日课 · 约三十分钟</p>
              <h2 className="mt-0.5 font-serif text-xl font-semibold">
                {today ? formatChineseDate(today) : "今日"}
              </h2>
            </div>
            <p className="font-serif text-sm tabular-nums text-muted-foreground">
              {doneToday.length} / {DAILY_ITEMS.length}
            </p>
          </header>
          <ChecklistBody
            items={DAILY_ITEMS}
          ...
          <footer className="border-t border-border px-5 py-3 text-sm text-muted-foreground sm:px-6">
            {allDailyDone
              ? "今天的日课已完成。贵在不断，不贵在多——明天再来。"
              : "中断不是失败，中断之后不回来才是。做不完就做一项，但每天做。"}
          </footer>
        </section>
        ...
        <section className="rounded-xl bg-card ring-1 ring-border">
          <header className="flex flex-wrap items-end justify-between gap-3 border-b border-border px-5 py-4 sm:px-6">
            <div>
              <p className="text-xs tracking-widest text-muted-foreground">周课 · 约一小时，建议周日</p>
              <h2 className="mt-0.5 font-serif text-xl font-semibold">本周</h2>
            </div>
            <p className="font-serif text-sm tabular-nums text-muted-foreground">
              {doneWeek.length} / {WEEKLY_ITEMS.length}
            </p>
          </header>
          <ChecklistBody
            items={WEEKLY_ITEMS}
      ...
      <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-serif text-2xl font-semibold tabular-nums">
                {streak} <span className="text-sm font-normal text-muted-foreground">天</span>
              </p>
            </div>
            <div className="ml-auto text-right">
              <p className="text-xs text-muted-foreground">累计</p>
              <p className="font-serif text-2xl font-semibold tabular-nums">
                {totalDays} <span className="text-sm font-normal text-muted-foreground">天</span>
              </p>
            </div>
          </div>
          <div className="mt-5">
            <p className="mb-2 text-xs text-muted-foreground">近十四天</p>
            <div className="grid grid-cols-14 gap-1">
              {recentDays.map((day) => (
                <span
                  key={day.key}
                  title={`${day.key}：${day.done} / ${DAILY_ITEMS.length}`}
                  className={cn(
        ...
        <div className="rounded-xl bg-paper-deep p-5 text-sm leading-relaxed">
          <p className="font-serif text-base font-semibold">三条提醒</p>
          <ul className="mt-2 space-y-2 text-muted-foreground">
            <li>你会中断。中断了，第二天照做，不补，不自责。</li>
            <li>进步是看不见的。心性的变化以年为单位。</li>
            <li>手册会过时，你不会。留下有用的，扔掉其余的。</li>
          </ul>
          <Link
