    ...
    return (
      <div className="mx-auto max-w-2xl">
        <div className="mb-8">
          ...
          <Progress value={(answered / QUESTIONS.length) * 100} aria-label="作答进度">
        ...
        <div className="mt-8 flex items-center justify-between gap-3">
          ...
          {page < pages.length - 1 ? (
            ...
            <Button disabled={!isComplete(answers)} onClick={finish}>
              查看结果
              <ArrowRightIcon data-icon="inline-end" />
            </Button>
          )}
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="rounded-xl bg-card p-6 ring-1 ring-border sm:p-8">
        <h2 className="font-serif text-xl font-semibold">开始之前</h2>
        <ul className="mt-4 space-y-3 text-sm leading-relaxed text-muted-foreground sm:text-base">
          <li className="flex gap-3">
            <span className="font-serif text-primary">一</span>
            <span>
              共二十四题，分四组，约五分钟。每题描述一种<strong className="text-foreground">做法</strong>，请按你实际做的频率作答，而不是你知道该怎么做。
            </span>
          </li>
          <li className="flex gap-3">
            <span className="font-serif text-primary">二</span>
            <span>
              结果不是分数，是镜子：你在智商与情商两条路上各处于哪一阶，以及你的「错位」形状。
            </span>
          </li>
          <li className="flex gap-3">
            <span className="font-serif text-primary">三</span>
            <span>
              结果只保存在你本机的浏览器里，不会上传。建议每季度重做一次，对照变化。
            </span>
          </li>
        </ul>
        <div className="mt-6 flex flex-wrap gap-3">
          <Button size="lg" className="h-10 px-5" onClick={start}>
            开始自测
            <ArrowRightIcon data-icon="inline-end" />
          </Button>
          {hydrated && lastResult && (
            <Button size="lg" variant="outline" className="h-10 px-5" onClick={showLast}>
              查看上次结果（{formatDate(lastResult.takenAt)}）
            </Button>
          )}
        </div>
      </div>
      <p className="mt-6 text-center text-sm text-muted-foreground">
        想先了解四个阶段的含义？读
        <Link href="/manual/know-thyself" className="mx-1 text-primary underline underline-offset-4">
          第三章 · 自知
        </Link>
        。
      </p>
    ...
    <div className="mx-auto max-w-3xl space-y-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          测于 {formatDate(result.takenAt)}
          {previous && (
            <span className="ml-2">· 与 {formatDate(previous.takenAt)} 的结果对照</span>
          )}
        </p>
          ...
          return (
            <div key={path} className="rounded-xl bg-card p-6 ring-1 ring-border">
              <p className="text-xs tracking-widest text-muted-foreground">
                {PATH_NAMES[path]} · 当前阶段
              </p>
              <p className={cn("mt-1 font-serif text-4xl font-semibold", LEVEL_TEXT[stage])}>
                {STAGE_NAMES[stage]}
              </p>
              <p className="mt-1 text-sm text-muted-foreground">
                平均 {result.pathAverages[path].toFixed(1)} / 5 ·{" "}
                {stage === "dao"
                  ? "四层都已稍稍稳固；这一阶没有终点"
                  : `「${LEVELS[stage].name}」这一层尚未稳固`}
              </p>
              <p className="mt-4 text-sm leading-relaxed">{STAGE_ADVICE[stage][path]}</p>
            </div>
          );
        })}
      </section>
      ...
      <section className="rounded-xl bg-card p-6 ring-1 ring-border">
        <h2 className="font-serif text-lg font-semibold">八格图</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          每格为三题平均分（1–5）。3.5 以上视为稍稍稳固。
        </p>
        <div className="mt-6 grid gap-8 sm:grid-cols-2">
          {(["iq", "eq"] as Path[]).map((path) => (
            <div key={path}>
              <p className="mb-3 font-serif text-sm font-semibold">{PATH_NAMES[path]}</p>
                    ...
                    <li key={level}>
                      <div className="mb-1 flex items-center justify-between text-sm">
                        ...
                        >
                          ...
                          <span className="text-muted-foreground">{LEVELS[level].short}</span>
                        </Link>
      ...
      {/* Misalignments */}
      <section>
        <h2 className="font-serif text-lg font-semibold">形状诊断</h2>
        {result.misalignments.length === 0 ? (
          <p className="mt-3 rounded-xl bg-card p-5 text-sm leading-relaxed ring-1 ring-border">
            八格之间没有明显错位——你的图是平的。这意味着接下来的功课不是补短，而是整体往上走一层：从你两条路各自的「当前阶段」开始。
          </p>
        ) : (
          <ul className="mt-3 space-y-3">
            {result.misalignments.map((m) => (
              <li key={m.key} className="rounded-xl bg-card p-5 ring-1 ring-border">
                <p className="font-serif text-base font-semibold text-primary">{m.title}</p>
      ...
      {/* Recommended */}
      <section>
        <h2 className="font-serif text-lg font-semibold">建议的阅读顺序</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          从你最薄弱的两格出发，加上针对错位的章节。读完一章，做一章，再读下一章。
        </p>
      ...
      <div className="flex flex-wrap gap-3 border-t border-border pt-8">
        ...
        <Button variant="outline" nativeButton={false} render={<Link href="/manual/practice-plan" />}>
          看修炼计划
        </Button>
      </div>
    </div>
